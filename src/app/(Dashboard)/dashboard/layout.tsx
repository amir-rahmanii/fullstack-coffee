...
async function LayOutDashboard({
  children,
}: Readonly<{
  ...
  const userDetails: UserType = await UserModel.findOne(
  ...
  return (
    <div className="bg-admin-black min-h-screen text-white">
      <HeaderPannel isAdminPannel={true} username={userDetails.username} />
      <SideBarPannel pathMenu={pathMenuDashboard} />
